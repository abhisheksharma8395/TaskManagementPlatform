package com.taskmanagement.app.labelservice.util;
...
public class JWTUtil {
    ...
    public Key getKey() { return Keys.hmacShaKeyFor(Decoders.BASE64.decode(secretKey)); }
    public String extractUsername(String t) { return extractClaim(t, Claims::getSubject); }
    public String extractRole(String t) { return extractClaim(t, c -> c.get("role", String.class)); }
    ...
    public <T> T extractClaim(String t, Function<Claims,T> r) { return r.apply(Jwts.parser().verifyWith((SecretKey)getKey()).build().parseSignedClaims(t).getPayload()); }
    public boolean validateToken(String t) { try { return !extractClaim(t,Claims::getExpiration).before(new Date()); } catch(Exception e) { return false; } }
}
